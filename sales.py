...
if "logged_in" not in st.session_state:    # login page
    st.session_state.logged_in = False

if "user" not in st.session_state:          # user role
    st.session_state.user = None

if "page" not in st.session_state:          #login page
    st.session_state.page = "login"

if "form_id" not in st.session_state:       # to help form to reset
     st.session_state.form_id = 0

if "start_form" not in st.session_state:    # to start a form
    st.session_state.start_form = False

# Login Page function:
def login():
    st.sidebar.markdown(
    "<h1 style='color:red; font-family:Arial, font-align:left'>Sales Intelligence Hub</h1>",
...
def add_sale():    
    ...
    with st.form(f"sales_form_{st.session_state.form_id}"):
            if role == "Super Admin":
                ...
                if st.form_submit_button("Get ID") and branch_name != "None":
                     branch_id=branch[branch_name]
                     st.success(f"Branch ID: {branch_id}")
                else:
                     st.error("Invalid Branch")
                branch_id=branch[branch_name]
        ...
        index=0)
            ...
            if submitted:
                if branch_id ==0:
                     st.warning("Enter a valid branch")
...
def add_payment():                       
    ...
    with st.form(f"sales_form_{st.session_state.form_id}",clear_on_submit=True):
        if role == "Super Admin":
            df_SA=pd.read_sql("SELECT branch_id,branch_name FROM branches",conn)
            b_l=df_SA.values.tolist()
            b_d=df_SA.to_dict(orient='records')
            print(b_d)
            a_n=st.selectbox("Select Branch Name:",df_SA["branch_name"])
            branch_id = next(
                (row["branch_id"] for row in b_d if row["branch_name"] == a_n),
                 None
                )
            ...
            branch_id=st.number_input("***Branch_ID:***", value= branch_id, disabled=True)
            a_n=st.text_input("***Branch_Name:***", value= r_ad[1], disabled=True)
        ...
        if sale_id and sub_pen:
            cur.execute("SELECT pending_amount FROM customer_sales WHERE sale_id=%s AND branch_id=%s", (sale_id,branch_id))
            ...
            if result:
                ...
                st.error(f"Invalid Sale ID.ID:{sale_id} is not available in {a_n}")
...
def dash_filter_SA():
    ...
    with col2:
        product=st.selectbox("Course",["All"]+ sorted(df["product_name"].unique()))
    with col3:
...
# SLIDER FILTER:
    ...
    df["pending_amount"] = df["pending_amount"].astype(float)
    
    max_gross = filtered_df["gross_sales"].max()
    received_min = 0.0
    received_max = filtered_df["received_amount"].max()
   
    rec_min, rec_max = st.slider(
        " 💰 Received Amount",
        received_min,
        received_max,
        value=(received_min, received_max)
    )
    final_df = filtered_df[filtered_df["received_amount"].between(rec_min, rec_max)]
# SHOW METRIC:
    col1,col2,col3 = st.columns(3)
    with col1:
    ...
    with col3:
        st.metric("Pending", final_df["pending_amount"].sum())   
...
#4. Option and Functions:   
    if access_to == "Dashboard":        #I
        if role and role.title() == "Super Admin": # title means case sensitive
            ...
            show_perf_A()
            show_perf1_A()
    elif access_to=="Add Sale":         #II        
            add_sale()   
    elif access_to== "Add Payment":     #III
            add_payment()
    elif access_to == "All Sales Data": #IV
            df = fetch_sales_SA()
            st.subheader("All Sales Data")
            st.dataframe(df)
    elif access_to == "Branch Sales":   #V
            df= fetch_branch_SA()
            ...
            for branch_id, data in df.groupby("branch_id"):
                ...
                st.dataframe(data)     
    elif access_to == "Payment Details":  #VI
            df= fetch_payment_detail()
            st.subheader("💵 Payment Data")
            st.dataframe(df)
    elif access_to == "SQL Queries":        #VII
         sql_qry()
